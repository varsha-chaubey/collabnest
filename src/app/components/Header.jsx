            ...
            <div className='flex items-center space-x-4'>
                ...
                <DropdownMenu open={open} onOpenChange={setOpen}>
                    <DropdownMenuTrigger asChild>
                        <Avatar className='cursor-pointer h-12 w-12 rounded-full'>
                            {session?.user?.image ? (
                                <AvatarImage className="rounded-full" src={session?.user?.image} alt={session?.user?.name} />
                            ) : (
...
<div className='flex flex-col items-center mb-4'>
    <Avatar className='cursor-pointer'>
        {session?.user?.image ? (
            <AvatarImage className="h-20 w-20" src={session?.user?.image} alt={session?.user?.name} />
        ) : (
            <AvatarFallback className="text-lg dark:bg-gray-300">{userPlaceHolder}</AvatarFallback>
        )}
    </Avatar>
